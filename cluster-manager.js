...
function listen(cmd, hookFunc)
{
	if (cluster.isMaster)
	{
		if (!hasForked)
		{
			...
			{
				preForkListeners[cmd] = [];
			}
			preForkListeners[cmd].push(hookFunc);
		}
		else
		{
			for (var k in cluster.workers)
			{
				...
				cluster.workers[k].on('message', (function(workerId)
				{
					return function(msg) {
...
function sendToWorker(workerId, cmd, data, done)
{
	if (!cluster.isMaster)
	{
